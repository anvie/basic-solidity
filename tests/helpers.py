...
def get_mint_sig(sender, to, count, admin, nonce, discount=True):
    m_hash = solidity_keccak(abi_types=["address", "address", "uint16", "uint64", "bool"],
                             values=[sender.address, to.address,
                                     count, nonce, discount],
                             validity_check=True)
...
def get_mint_sig_with_nonce(sender, to, count, admin, discount=True):
    nonce = get_nonce() + 1
    sig = get_mint_sig(sender, to, count, admin, nonce, discount)
    return sig, nonce
