...
def get_mint_sig(sender, to, count, admin_or_privkey, nonce, airdrop=False):
    ...
    if hasattr(admin_or_privkey, 'private_key'):
        sig = Account.signHash(msg_hash, admin_or_privkey.private_key)
    else:
        sig = Account.signHash(msg_hash, admin_or_privkey)
    return (sig.r, sig.s, sig.v)


def get_mint_sig_with_nonce(sender, to, count, admin_or_privkey, airdrop=False):
    nonce = get_nonce()
    sig = get_mint_sig(sender, to, count, admin_or_privkey, nonce, airdrop)
    return sig, nonce
